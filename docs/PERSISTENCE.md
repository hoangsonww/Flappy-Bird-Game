...
Where the game keeps things, how they are encoded, and what happens when the
format changes.
...
The profile storage key carries a version: `player.profile.v2`.
...
old documents decode fine when obsolete profile fields are removed.
